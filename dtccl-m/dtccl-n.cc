...
class MyWorker : public NanAsyncWorker {
 public:
  std::string inName;
  MyWorker(NanCallback *callback, MyObject* o1,std::vector<char> im,std::string name, int mode)
    : NanAsyncWorker(callback),o1(o1),im(im),name(name), mode(mode) {
    }
  ~MyWorker() {}
  ...
  //std::cout << "O2 from worker :" << o2->Num.ret() << std::endl;
    ...
    input = cv::imdecode(im,CV_LOAD_IMAGE_GRAYSCALE);
...
NAN_METHOD(Justdetect) {
  ...
  std::vector<char> im(bufferData, bufferData + bufferLength);
